    ...
    def __init__(
        ...
        if (
            ...
            self.git_icon.setPixmap(
                ModListIcons.git_icon().pixmap(QSize(20, 20))
            )
        ...
        if not self.git_icon and not self.steamcmd_icon:
            ...
            self.mod_source_icon.setPixmap(
                self.get_icon().pixmap(QSize(20, 20))
            )
            ...
            if action:  # Handle the action for all selected items
                if (  # ACTION: Update git mod(s)
                    ...
                    answer = show_dialogue_conditional(
                        ...
                        text=self.tr(
                            "You have selected {len} git mod(s) to be updated."
                    ...
                    answer = show_dialogue_conditional(
                        title=self.tr("Are you sure?"),
                        text=self.tr(
                            "You have selected {len} mod(s) for unsubscribe + re-subscribe."
                    ...
                    answer = show_dialogue_conditional(
                        title=self.tr("Are you sure?"),
                        text=self.tr(
                            "You have selected {len} mod(s) for unsubscribe. This also deletes the mod(s) and their dds textures."
                        ).format(len=len(publishedfileids)),
                        information=self.tr("\nDo you want to proceed?"),
                    )
                    if answer == "&Yes":
